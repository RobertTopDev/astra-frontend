...
export const convertUSD = (financialData: string | number) => {
  const financialDataNumercis: number =
    typeof financialData === 'string'
      ? parseFloat(financialData)
      : financialData
  return financialDataNumercis?.toLocaleString('en-US', {
    style: 'currency',
    currency: 'USD',
  })
}
