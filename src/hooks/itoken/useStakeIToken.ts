...
type TUseStakeITokenProps = {
  onSuccessTx?: (data: TransactionReceipt) => void
  onRevert?: (data: TransactionReceipt) => void
} & UsePrepareContractWriteConfig<typeof itokenStakingABI, 'deposit'>

export const useStakeIToken = ({
  onSuccessTx,
  onRevert,
  ...props
}: TUseStakeITokenProps) => {
  ...
  const {
    ...
    onMutate: () => {
      setTransactionObj({
        status: 'loading',
        reset,
        transactionAction: 'Staking iToken',
      })
    },
    onError: (error) => {
      setTransactionObj({
