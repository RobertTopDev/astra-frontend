...
type TUseRestakeAstraProps = {
  onSuccessTx?: (data: TransactionReceipt) => void
  onRevert?: (data: TransactionReceipt) => void
} & UsePrepareContractWriteConfig<typeof chefAbi, 'claimAstra'>

export const useRestakeAstra = ({
  onSuccessTx,
  onRevert,
  ...props
}: TUseRestakeAstraProps) => {
  ...
  const {
    ...
    onMutate: () => {
      setTransactionObj({
        status: 'loading',
        reset,
        transactionAction: 'Restaking Astra DAO',
      })
    },
    onError: (error) => {
      setTransactionObj({
  ...
  return {
    restake: props.enabled || props.enabled === undefined ? write : undefined,
    restakeAsync:
